...
async fn collect_file_send_list(
    ...
    reply_rx
        .await
        .map_err(|_| io::Error::other(format!("CollectFileSendList for peer {peer_id} dropped")))?
}
...
async fn stream_send_list(
    ...
    for (entry, path_opt) in send_list {
        ...
        write_frame(send, &header).await.map_err(|err| {
            sync_io_error(format!("{context}: write FileHeader for {}", entry.id), err)
        })?;
        if size > 0 {
            if let Some(path) = path_opt {
                write_file_to_stream(send, path).await.map_err(|err| {
                    sync_io_error(format!("{context}: stream content for {}", entry.id), err)
                })?;
            }
        }
    }
...
async fn stream_recv_list(
    ...
    for i in 0..count {
        ...
        let temp_path = if header.size > 0 {
            ...
            if let Err(err) = read_stream_to_temp(recv, header.size, &header.hash, &tmp).await {
    ...
    loop {
        ...
        if let RequestMessage::JoinRequest { token, joiner_id } = request {
            ...
            if accepted {
                ...
                if !members.iter().any(|entry| entry.id == self_id_str) {
                ...
                close_send(&mut send).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: close send (join accepted)"),
                        err,
                    )
                })?;
                ...
                close_send(&mut send).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: close send (join rejected)"),
                        err,
                    )
                })?;
        ...
        match request {
            RequestMessage::GetNodes {
                ...
                assert_eof(&mut recv).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: trailing bytes after GetNodes"),
                        err,
                    )
                })?;
                ...
                close_send(&mut send).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: close send (nodes #{request_id})"),
                        err,
                    )
                })?;
            }
            RequestMessage::GetFiles { request_id, ids } => {
                assert_eof(&mut recv).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: trailing bytes after GetFiles"),
                        err,
                    )
                })?;
                ...
                close_send(&mut send).await.map_err(|err| {
                    sync_io_error(
                        format!("incoming peer {peer}: close send (files #{request_id})"),
                        err,
                    )
                })?;
            }
...
#[derive(Debug, Clone)]
pub(crate) struct FolderProtocol {
    pub tx: mpsc::UnboundedSender<Command>,
    pub allowlist: Arc<RwLock<HashSet<PublicKey>>>,
    pub invites_path: Arc<Path>,
    pub peers_path: Arc<Path>,
    pub local_id: PublicKey,
}

impl ProtocolHandler for FolderProtocol {
...
pub(crate) async fn fetch_entries(
    ...
    request_files(endpoint, peer, ids, tmp_dir).await
}
...
async fn request_files(
    ...
    let (mut send, mut recv) = connection.open_bi().await.map_err(|err| {
        sync_io_error(format!("peer {peer_id}: open bi stream for GetFiles"), err)
    })?;
    ...
    close_send(&mut send).await.map_err(|err| {
        sync_io_error(
            format!("peer {peer_id}: close send (GetFiles #{request_id})"),
            err,
        )
    })?;

    let resp: ResponseMessage = read_frame(&mut recv).await.map_err(|err| {
        sync_io_error(
            format!("peer {peer_id}: read FilesBegin #{request_id}"),
            err,
        )
    })?;
...
pub(crate) async fn send_join_request(
    ...
    host_id: PublicKey,
    token: &str,
) -> io::Result<(Option<String>, Vec<MemberEntry>)> {
